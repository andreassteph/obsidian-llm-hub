...
function displaySelectedRagSetting(
  ...
  // Store Mode Toggle
  const currentMode = ragSetting.isLocal ? "local" : ragSetting.isExternal ? "external" : "internal";
  new Setting(containerEl)
    ...
    .addDropdown((dropdown) =>
      dropdown
        ...
        .addOption("local", t("settings.storeModeLocal"))
        .setValue(currentMode)
        .onChange((value) => {
          void (async () => {
            if (value === "external") {
              await plugin.updateRagSetting(name, { isExternal: true, isLocal: false, storeId: null, storeName: null });
            } else if (value === "local") {
              await plugin.updateRagSetting(name, { isExternal: false, isLocal: true, storeId: null, storeName: null });
            } else {
              await plugin.updateRagSetting(name, { isExternal: false, isLocal: false, storeId: null, storeName: null });
            }
...
function displayLocalStoreSettings(
  ...
  new Setting(containerEl)
    ...
    .addDropdown((dropdown) => {
      ...
      for (const m of models) dropdown.addOption(m, m);
      dropdown.setValue(plugin.settings.localRagEmbeddingModel)
        .onChange((value) => {
          void (async () => {
            plugin.settings.localRagEmbeddingModel = value;
            await plugin.saveSettings();
          })();
        });
    });
