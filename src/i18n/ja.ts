...
export const ja: Record<string, string> = {
  ...
  "settings.apiProviderModel": "デフォルトモデル",
  "settings.apiProviderModel.desc": "使用するモデルID（例：gpt-4o、claude-3-opus）。「確認」をクリックして利用可能なモデルを検出できます。",
  ...
  // Settings - RAG Store
  "settings.storeModeLocal": "ローカル埋め込み",
  "settings.storeModeLocal.desc": "Gemini Embedding APIを使用してローカルベクトルインデックスを作成します。CLIを含むすべてのバックエンドで動作します。",
  ...
  "settings.localEmbeddingApiKey.desc": "埋め込みサーバー用のカスタムAPIキー（Google APIキーを使用する場合は空白のまま）",
  ...
  "input.ragLocal": "\u{1F50D} {{name}} (ローカル)",
  ...
  "aiWorkflow.pastePlaceholder": "Claude、GPT等が出力したMarkdown（```llm-hub-workflowブロック含む）またはYAMLを貼り付け...",
