...
**RAG Search** タブ（Chat と Workflow の間）は、RAG インデックスの検索と閲覧のための専用インターフェースを提供します。

**検索機能：**
- RAG 設定の選択、検索ごとに Top K とスコア閾値を調整
- テキスト結果は展開可能なアコーディオンで表示（クリックで全文表示）
- PDF 結果はインライン PDF ページプレビューで表示（抽出されたチャンクページ）
- **インデックス設定**（歯車アイコン）— チャンクサイズ、チャンクオーバーラップ、PDF分割ページ数、対象フォルダ、除外パターンを検索タブから直接編集可能。**同期**ボタン、プログレスバー、最終同期日時、ファイルごとのチャンク数付き登録ファイル一覧を含む

**検索結果を Chat に送信：**
1. チェックボックスで結果を選択（または「すべて選択」）
2. **Chat with selected** をクリック
3. 結果が Chat 入力エリアに添付ファイルとして追加
4. Chat の RAG ドロップダウンが自動的に「none」に設定され、RAG の二重注入を防止

**添付ファイルの編集：**
- Chat 入力エリアのテキスト添付ラベルをクリックするとモーダルで開く
- テキストを編集して保存 — 送信前に添付内容が更新される

**PDF 結果の処理：**
- **内部 RAG**（このプラグインでインデックス済み）：PDF は抽出されたページチャンク（実際の PDF ページ）として添付
- **外部 RAG**（抽出テキスト付きの事前構築インデックス）：結果ごとのドロップダウンで「テキストとして」（編集可能）または「PDF チャンクとして」（ページ抽出）を選択可能

**外部ファイルリンク：** 検索結果でファイルパスをクリックすると、Vault ファイルは Obsidian で、外部ファイルは OS のデフォルトアプリケーションで開きます。

> RAG 設定が存在しない場合、タブにはプラグイン設定へのリンク付きセットアップガイドが表示されます。
